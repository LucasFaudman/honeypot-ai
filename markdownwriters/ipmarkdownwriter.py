...
class IPMarkdownWriter(MarkdownWriterBase):
    ...
    def add_isc_ip_locations(self, md, attack: Attack):
        ...
        ip_loc_md += table(['IP Address', 'Country', "AS",
                           "AS Name", "Network"], location_data)
    ...
    def add_ip_locations(self, md, attack: Attack):
        ...
        location_headers = ['IP Address', 'Country',
                            "City", "ISP", "Organization",  "ASN", "Network"]
    ...
    def add_isc(self, md, attack: Attack):
        ...
        if len(data) == 1:
            ...
            sharing_link = link("https://isc.sans.edu/ipinfo/",
                                "https://isc.sans.edu/ipinfo/")
        ...
        isc_headers = ['IP Address', 'Total Reports', "Targets",
                       "First Report", "Last Report", "Update Time"]
        ...
        for key in isc_counts.keys():
            if key.startswith("as") or key.startswith("cloud") or key in ["network", "threatfeeds"]:
                isc_md += most_common_table(
                    key, isc_counts[key], n=10, style_fn=code, header_level=4)
    ...
    def add_whois(self, md, attack: Attack):
        ...
        for ip in data:
            ...
            sharing_link = link(
                whois_data["sharing_link"], whois_data["sharing_link"])

            ip_md = h3(f"Whois data for: {ip} " + sharing_link)
            ip_md += codeblock(whois_data["results"].get(
                "whois_text", whois_data["error"]))
    ...
    def add_cybergordon(self, md, attack: Attack):
        ...
        combined_headers = ["IP Addresss",
                            "Alerts High | Med | Low"] + all_engines
        combined_results = []
        ...
        for ip in data:
            ...
            sharing_link = link(
                cybergordon_data["sharing_link"], cybergordon_data["sharing_link"])
            ...
            for priority in ["high", "medium", "low"]:
                for entry in cybergordon_data["results"].get(priority, []):
                    cybergordon_table_data.append(
                        (entry["engine"], entry["result"], entry["url"]))
    ...
    def add_shodan(self, md, attack: Attack):
        ...
        combined_headers = ["IP Addresss", "# Open Ports"] + \
            sorted(shodan_counts["ports"].keys(), key=int)
        combined_results = []
        ...
        for ip in data:
            ...
            sharing_link = link(
                shodan_data["sharing_link"], shodan_data["sharing_link"])

            ip_md = h3(f"Shodan results for: {ip} " + sharing_link)

            headers = list(shodan_data["results"]["general"].keys())
            shodan_general_values = list(
                shodan_data["results"]["general"].values())
            ip_md += table(headers, [shodan_general_values])

            ip_md += h4("Open Ports")
            all_ports_table_headers = [
                "Port", "Protocol", "Service", "Update Time"]
            all_ports_data = []
            ports_md = ""

            combined_row = [ip, f"{code(len(shodan_data['results']['ports']))}"] + [
                "-" for _ in shodan_counts["ports"]]

            for port, entry in shodan_data["results"]["ports"].items():
                all_ports_data.append(
                    (port, entry["protocol"], entry["service_name"], entry["timestamp"]))
                ...
                port_md += table(["Key", "Value"], [(key.strip(), entry["service_data"][key])
                                 for key in entry["service_data"]])
                ports_md += port_md

                combined_row[combined_headers.index(
                    port)] = entry["service_name"]
            ...
            shodan_md += collapseable_section(ip_md + ports_md,
                                              # + sharing_link,
                                              f"Shodan results for: {ip}",
                                              header_level=3,
                                              )

        combined_results.sort(key=lambda x: len(
            data[x[0]]["shodan"]["results"]["ports"]), reverse=True)
        # int(x[1].replace('`','')))
        ...
        summary.append(
            f"The IP address with the {bold('most open ports')} was {code(max_open_ports[0])} with {code(max_open_ports[1])} open ports.")
    ...
    def add_threatfox(self, md, attack: Attack):
        ...
        for ip in data:
            ...
            sharing_link = link(
                threatfox_data["sharing_link"], threatfox_data["sharing_link"])
            ...
            for ioc in threatfox_data["results"]:
                ...
                threatfox_ioc_table_values.append(
                    (ioc_link, ioc_data["Threat Type"], malware_link, ioc_data["Malware alias"], ioc_data["Confidence Level"], ioc_data["First seen"], ioc_data["Last seen"]))

                malware_data = ioc["malware_data"]
                threatfox_malware_table_values.append(
                    (malware_link, malware_data["Malware alias"], malware_data["Number of IOCs"], malware_data["First seen"], malware_data["Last seen"], malware_data["Malpedia"]))

            if threatfox_ioc_table_values:
                ip_md += h4(
                    f"ThreatFox IOCS (Indicators of Compromise) Found for {ip}")
                threatfox_ioc_table_headers = [
                    "IOC", "Threat Type", "Malware", "Malware Alias", "Confidence Level", "First Seen", "Last Seen"]
                ip_md += table(threatfox_ioc_table_headers,
                               threatfox_ioc_table_values)

            if threatfox_malware_table_values:
                ip_md += h4(f"ThreatFox Malware Found for {ip}")
                threatfox_malware_table_headers = [
                    "Malware", "Aliases", "Number of IOCs", "First Seen", "Last Seen", "Malpedia URL"]
                ip_md += table(threatfox_malware_table_headers,
                               set(threatfox_malware_table_values))

            threatfox_md += collapseable_section(ip_md,
                                                 # + sharing_link,
                                                 f"ThreatFox results for: {ip}",
                                                 header_level=3,
                                                 )
