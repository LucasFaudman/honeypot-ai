...
class Session(SmartAttrObject, CachedPropertyObject):
    ...
    def process_events(self):
        ...
        if self.events:
            ...
            self.duration = (
                self.end_time - self.start_time).microseconds / 1000000
            ...
            self.protocol = self.events[0].get(
    ...
    def add_http_request(self, event):
        ...
        # TODO make indescrimate between Zeek and Web log events
        request_keys = ("timestamp", "method", "uri", "version",
                        "user_agent", "host", "referrer", "cookies")
        http_request = {key: event[key]
                        for key in request_keys if key in event}
    ...
    def cmdlog_hash(self):
        ...
        if self.commands:
            ...
            standardized_cmdlog = standardize_by_regexes(
                self.cmdlog, self.STANDARDIZE_REGEX_COMMANDS)
    ...
    def http_request_strs(self):
        ...
        for event in self._http_requests:
            ...
            http_request += "\n".join(
                [f"{key.title().replace('_','-')}: {event[key]}" for key in event if not key in skip_keys])
    ...
    def httplog_hash(self):
        ...
        if self._http_requests:
            standardize_by_regexes(
                self.httplog, self.STANDARDIZE_REGEX_HTTP_REQUESTS)
