...
class Attack(SmartAttrObject, CachedPropertyObject, PostprocessableObject):
    ...
    def __init__(self, attack_id, attack_id_type, source_ip) -> None:
        ...
        self._malware = {
            malware.id: malware for malware in source_ip.all_malware}
        self.log_paths = {}
        self._log_counts = {}
    ...
    def standardized_malware(self):
        ...
        for malware in self._malware.values():
            self._standardized_malware[malware.standardized_hash].append(
                malware)
    ...
    def update_malware_explanations(self, standardized_malware_explanations):
        ...
        self.standardized_malware_explanations.update(
            standardized_malware_explanations)
...
def recursive_split_commands(commands):
    ...
    block_regex = re.compile(
        r"if .+?; then.+?fi;?|(?:for|while) .+?; do.+?done;?|case .+?esac;?")
    ...
    for command in commands:
        ...
        if ";" in command and not 'awk' in command:
            split_commands.extend(cmd_part.strip()
                                  for cmd_part in command.split(";") if cmd_part.strip())
        elif command:
