...
DEFAULT_CONFIG = {
    # Sort settings
    # Order of attrs to sort attacks by
    "SORT_ATTRS": ["num_source_ips", "num_sessions", "num_commands", "num_malware", "num_http_requests"],
    "SORT_ORDER": "desc",  # Order to sort attacks by (asc or desc)
    # Maximum number of workers to use when loading attacks from the attacks directory
    ...
    # Whether or not to keep unset fields in Zeek logs
    ...
    # Minimum number of commands used for SourceIP included in an Attack
    "ATTACK_MIN_COMMANDS": 1,
    # Minimum number of malware files for SourceIP included in an Attack
    "ATTACK_MIN_MALWARE": 1,
    # Minimum number of successful logins for SourceIP included in an Attack
    "ATTACK_MIN_SUCCESSFUL_LOGINS": 1,
    # Minimum number of HTTP requests for SourceIP included in an Attack
    ...
    # Regexes to match in HTTP requests that should be standardized before hashing. All captured groups will be replaced with X before hashing.
    ...
    # Regexes to match in malware of attacks that should be merged
    'MERGE_REGEX_MALWARE': [],
    ...
    # Oranizer Settings
    # Whether or not to overwrite existing attack directories when organizing
    "ORGANIZER_OVERWRITE": True,
    # How to iterate when organizing (logs or attacks)
    "ORGANIZER_ITERBY": "logs",
    # How to run organizing concurrently (processes or threads)
    "ORGANIZER_CONCURRENCY_TYPE": "multiprocessing",
    # Maximum number of workers (processes or threads) to use when organizing
    "ORGANIZER_MAX_WORKERS": None,
    "ORGANIZER_CHUNKSIZE": 1,  # Chunksize to use when organizing
    # Order to yield results when organizing (as_completed or as_submitted)
    "ORGANIZER_YIELD_ORDER": "as_completed",
    # Whether or not to organize attacks into subdirectories by IP
    "ORGANIZER_IP_SUBDIRS": False,

    # OpenAI Settings
    "USE_OPENAI": True,  # Whether or not to run the OpenAIAnalyzer
    # Whether or not to use the OpenAI Code Interpreter
    "USE_OPENAI_CODE_INTERPRETER": True,
    # OpenAI API Key (Get from https://platform.openai.com/api-keys)
    "OPENAI_API_KEY": "<PASTE YOUR API KEY HERE>",
    # OpenAI Model to use (Get from https://platform.openai.com/docs/models)
    "OPENAI_MODEL": "gpt-4-1106-preview",
    # Path to the openai-training-data directory
    ...
    # Sources to use for the IPAnalyzer
    "IPANALYZER_SOURCES": ["isc", "whois", "cybergordon", "threatfox", "shodan"],
    # Maximum number of errors allowed before a source is skipped
    "IPANALYZER_MAX_ERRORS": 5,
    # Path to the webdriver executable for use with Selenium via SouperScraper
    "WEBDRIVER_PATH": "./resources/chromedriver",
    # Type of webdriver executable for use with Selenium via SouperScraper (chrome, firefox, edge, safari, etc.)
    "WEBDRIVER_TYPE": "chrome",

    # MalwareAnalyzer Settings
    "USE_MALWAREANALYZER": True,  # Whether or not to run the MalwareAnalyzer
    # Sources to use for the MalwareAnalyzer
    "MALWAREANALYZER_SOURCES": ["exploitdb", "malpedia", "malwarebazaar", "threatfox", "urlhaus"],
    # Maximum number of errors allowed before a source is skipped
    "MALWAREANALYZER_MAX_ERRORS": 5,
    # Weather or not to malware analyzer to attempt to download failed malware samples from Urlhaus
    ...
    # External IPs of the honeypot system(s) to inform AI for more accurate analysis
    "HONEYPOT_EXTERNAL_IPS": [],
    # Interal IPs of the honeypot system(s) to inform AI for more accurate analysis
    "HONEYPOT_INTERNAL_IPS": [],
    # Open ports of on honeypot system(s) to inform AI for more accurate analysis
    ...
    # Path to the cowrie logs directory (Should be a subdirectory of LOGS_PATH)
    "COWRIE_LOGS_PATH": "./logs/cowrie",
    # Path to the firewall logs directory (Should be a subdirectory of LOGS_PATH)
    "FIREWALL_LOGS_PATH": "./logs/firewall",
    # Path to the web logs directory (Should be a subdirectory of LOGS_PATH)
    "WEB_LOGS_PATH": "./logs/web",
    # Path to the zeek logs directory (Should be a subdirectory of LOGS_PATH)
    "ZEEK_LOGS_PATH": "./logs/zeek",
    # Path to the malware downloads directory (Should be a subdirectory of LOGS_PATH)
    "MALWARE_DOWNLOADS_PATH": "./logs/malware/downloads",
    # Path to the auth_random.json file (Should be a subdirectory of LOGS_PATH)
    ...
    # Output Paths
    # Path to the attacks directory where Attack data will be stored and loaded from
    "ATTACKS_PATH": "./attacks",
    # Path to the db directory where IP, Malware, and OpenAI data will be stored and loaded from
    "DB_PATH": "./db",
    # Path to the ipdb directory where IP data will be stored and loaded from (Should be a subdirectory of DB_PATH)
    "IPDB_PATH": "./db/ipdb",
    # Path to the mwdb directory where Malware data will be stored and loaded from (Should be a subdirectory of DB_PATH)
    "MWDB_PATH": "./db/mwdb",
    # Path to the aidb directory where OpenAI data will be stored and loaded from (Should be a subdirectory of DB_PATH)
    "AIDB_PATH": "./db/aidb",
    # Path to the reports directory where attack markdown reports and files will be exported too
...
def config_arg_parser():
    parser = argparse.ArgumentParser(
        description='honeypot-ai: Honeypot Log Analyzer Built on OpenAI')
    ...
    groups['Actions'].add_argument(
        '--list-attacks', '--list', '-L', action='store_true', help='List loaded attacks')
    groups['Actions'].add_argument('--print-attrs', '--print', '-P', metavar='ATTACK_ATTRS',
                                   type=str, nargs='+', action='extend', help='Print specified attributes of loaded attacks')
    groups['Actions'].add_argument('--organize-attacks', '--organize', '-O',
                                   action='store_true', help='Organize attacks into attack directories')
    groups['Actions'].add_argument('--analyze-attacks', '--analyze', '-A', action='store_true',
                                   help='Analyze loaded attacks with OpenAI and OSINT Analyzers')
    groups["Actions"].add_argument("--chat", "-C", action="store_true",
                                   help="Chat with the OpenAI Assistant about the loaded attacks")
    groups['Actions'].add_argument('--write-reports', '--write', '-W',
                                   action='store_true', help='Write markdown reports for analyzed attacks')
    groups["Actions"].add_argument('--export-reports', '--export', '-E',
                                   action='store_true', help='Export attack report and files to REPORTS_PATH')
    groups["Actions"].add_argument('--interactive', '--interact', '-I', action='store_true',
                                   help='Enter interactive mode after loading attacks (python shell with loaded attacks in the "ATTACKS" variable)')

    groups['Config File'].add_argument(
        '--config', '-c', metavar="FILE", type=str, default='config.json', help='Path to config file')
    groups['Config File'].add_argument(
        '--update-config', '-u', action='store_true', help='Update config file with new values')

    groups["Loading Attacks"].add_argument(
        '--load-from-logs', '-lfl', '-ll', action='store_true', help='Load attacks from logs')
    groups["Loading Attacks"].add_argument(
        '--load-from-attacks-dir', '-lfa', '-la', action='store_true', help='Load attacks from attacks directory')
    groups["Loading Attacks"].add_argument('--only-attacks', metavar='ATTACK_IDS',
                                           type=str, nargs='+', default=None, help='Only load attacks with these keys')
    groups["Loading Attacks"].add_argument('--skip-attacks', metavar='ATTACK_IDS',
                                           type=str, nargs='+', default=None, help='Skip loading attacks with these keys')
    groups["Loading Attacks"].add_argument(
        '--max-ips-per-attack', type=int, default=None, help='Maximum number of IPs in each loaded attack')
    groups["Loading Attacks"].add_argument(
        '--max-attacks', type=int, default=None, help='Maximum number of attacks to load')
    ...
    for key, value in DEFAULT_CONFIG.items():
        ...
        if isinstance(value, (list, dict)):
            add_argument_kwargs["type"] = type(
                value[0]) if len(value) > 0 else str
        ...
        groups.get(group_name, parser).add_argument(
            *arg_names, **add_argument_kwargs)
...
def main(test_args=None):
    ...
    if args.config:
        try:
            with open(args.config, 'r') as f:
                config.update(
                    {k: v for k, v in json.load(f).items() if k in config})
    ...
    config.update({k.upper(): v for k, v in vars(args).items()
                   # Not an unset arg (Note: config values cannot be set to None on the command line but can be by editing the config file.)
                   if v != None
                   # Not the same as the value in the config file
                   and v != config.get(k.upper(), v)
    ...
    Session.STANDARDIZE_REGEX_COMMANDS = list(
        map(re.compile, config["STANDARDIZE_REGEX_COMMANDS"]))
    Session.STANDARDIZE_REGEX_HTTP_REQUESTS = list(
        map(re.compile, config["STANDARDIZE_REGEX_HTTP_REQUESTS"]))
    Malware.STANDARDIZE_REGEX_MALWARE = [re.compile(
        regex.encode()) for regex in config["STANDARDIZE_REGEX_MALWARE"]]
    ...
    LOG_PROCESSOR = LogProcessor(
        parsers=LOG_PARSERS,
        remove_ips=config["USER_IPS"] + config["HONEYPOT_INTERNAL_IPS"] +
        config["HONEYPOT_EXTERNAL_IPS"],
    ...
    if args.load_from_attacks_dir:
        print(
            f"Loading attacks from attacks directory at {config['ATTACKS_PATH']}")
    ...
    for attack in list(ATTACKS.values()):
        ...
        if args.max_ips_per_attack and attack.num_source_ips > args.max_ips_per_attack:
            ...
            print(
                f"Skipping attack {attack.attack_id} with {attack.num_source_ips} IPs (max_ips_per_attack={args.max_ips_per_attack})")
        # Remove attacks not in only_attacks if --only-attacks flag is set
            ...
            del ATTACKS[attack.attack_id]
            print(
                f"Skipping attack {attack.attack_id} (not in only_attacks={args.only_attacks})")
        # Remove attacks in skip_attacks if --skip-attacks flag is set
            ...
            del ATTACKS[attack.attack_id]
            print(
                f"Skipping attack {attack.attack_id} (in skip_attacks={args.skip_attacks})")

    # Remove attacks after max_attacks if --max-attacks flag is set
    if args.max_attacks and args.max_attacks < len(ATTACKS):
        print(
            f"Skipping {len(ATTACKS) - args.max_attacks} attacks (max_attacks={args.max_attacks})")
    ...
    if args.organize_attacks and ATTACKS:
        ATTACK_DIR_ORGANIZER = AttackDirOrganizer(
            attacks=ATTACKS,
            # Use CowrieParser so auth_random.json is organized
            parser=CowrieParser(config["LOGS_PATH"]),
        ...
        print(
            f"Organizing attacks into attack directories at {config['ATTACKS_PATH']}")
        for result in results_generator:
            print(result)
        print(
            f"Finished organizing attacks into attack directories at {config['ATTACKS_PATH']}")
    ...
    if args.chat:
        ...
        for attack in ATTACKS.values():
            print(
                f"Chatting about attack {attack.attack_id} with OpenAI Assistant")
            question_run_logs = OPENAI_ANALYZER.interactive_chat_about_attack(
                attack)
            attack.question_run_logs.update(question_run_logs)
            print(
                f"Finished chatting about attack {attack.attack_id} with OpenAI Assistant")

            if not question_run_logs:
                print("No questions asked. Skipping writing chat run steps markdown")
                continue

            question_keys = sorted(attack.question_run_logs.keys(), key=lambda k: (
                int(k.split("_")[-1]), k) if k.split("_")[-1].isdigit() else (0, k))
            file_prefix = f"{question_keys[0]}-{question_keys[-1]}" if len(
                question_keys) > 1 else question_keys[0]
            chat_run_steps_filepath = attack.attack_dir / \
                "ai-chat" / f"{file_prefix}-run-steps.md"

            print(
                f"Writing chat markdown run steps for attack {attack.attack_id} to {chat_run_steps_filepath}")
            ...
            print(
                f"Finished writing chat markdown run steps for attack {attack.attack_id} to {chat_run_steps_filepath}")
    ...
    if args.write_reports or args.export_reports:
        for attack in ATTACKS.values():
            ...
            print(
                f"Writing markdown report for attack {attack.attack_id} to {report_filepath}")
            ...
            REPORT_MARKDOWN_WRITER.convert_report_md_to_txt_for_canvas(
                attack, report_filepath)
            print(
                f"Finished writing markdown report and for attack {attack.attack_id} to {report_filepath}")

            run_steps_filepath = attack.attack_dir / "run-steps.md"
            print(
                f"Writing markdown run steps for attack {attack.attack_id} to {run_steps_filepath}")
            RUN_STEPS_MARKDOWN_WRITER = RunStepsMarkdownWriter(
            ...
            RUN_STEPS_MARKDOWN_WRITER.update_md()
            print(
                f"Finished writing markdown run steps for attack {attack.attack_id} to {run_steps_filepath}")

            # Export attack report and files to REPORTS_PATH if --export-reports flag is set
            if args.export_reports:
                print(
                    f"Exporting attack report and files for attack {attack.attack_id} to {config['REPORTS_PATH']}")
                export_dir = config["REPORTS_PATH"] / attack.answers['title']
                copytree(attack.attack_dir, export_dir,  dirs_exist_ok=True)
                print(
                    f"Finished exporting attack report and files for attack {attack.attack_id} to {config['REPORTS_PATH']}")
