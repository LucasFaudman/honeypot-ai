...
DEFAULT_QUESTIONS = {
    ...
    "malware_osint_summary": "Explain what is known about the hosts, malware, and/or exploits used in the attack using data from MalwareBazaar, ThreatFox, URLhaus, and Malpedia. "
...
class AttackAnalyzer:
    ...
    def get_all_log_paths_and_counts(self, attacks=None):
        ...
        if not self.attack_dir_reader:
            raise MissingAnalyzerError(
                "AttackDirReader not initialized can't get log paths and counts.")

        log_paths = {}
        log_counts = {}
        for attack in attacks.values():
            attack.add_postprocessor(self.attack_dir_reader)
            attack_log_paths, attack_log_counts = self.attack_dir_reader.update_attack_log_paths_and_counts(
                attack)
    ...
    def get_all_ipdata(self, attacks=None):
        ...
        if not self.ip_analyzer:
            raise MissingAnalyzerError(
                "IPAnalyzer not initialized can't get ip data.")
    ...
    def get_all_mwdata(self, attacks=None):
        ...
        if not self.malware_analyzer:
            raise MissingAnalyzerError(
                "MalwareAnalyzer not initialized can't get mwdata.")
    ...
    def get_all_command_explanations(self, attacks=None):
        ...
        if not self.openai_analyzer:
            raise MissingAnalyzerError(
                "OpenAIAnalyzer not initialized can't get command explanations.")
        ...
        for attack in attacks.values():
            ...
            attack_command_explanations = self.openai_analyzer.explain_commands(
                attack.split_commands)
    ...
    def get_all_malware_explanations(self, attacks=None):
        ...
        if not self.openai_analyzer:
            raise MissingAnalyzerError(
                "OpenAIAnalyzer not initialized can't get malware explanations.")
    ...
    def get_all_assistant_answers(self, attacks=None):
        ...
        if not self.openai_analyzer:
            raise MissingAnalyzerError(
                "OpenAIAnalyzer not initialized can't get assistant answers.")
        ...
        for attack in attacks.values():
            ...
            question_run_logs = self.openai_analyzer.answer_attack_questions(
                questions, attack)
            # update run_logs with questions from chat. Allows for overwriting answers to default questions.
            chat_question_run_logs, question_num = self.openai_analyzer.read_attack_chat_run_logs(
                attack)
            ...
            attack.answers["title"] = attack.answers["title"].strip(
                '"').strip("'")

        return assistant_answers

    def get_all_failed_malware(self, attacks=None):
        attacks = attacks or self.attacks

        if not self.malware_analyzer:
            raise MissingAnalyzerError(
                "MalwareAnalyzer not initialized can't get malware.")
            ...
            if attack._malware:
                for malware_id, malware_obj in list(attack._malware.items()):
                    if malware_obj.failed and malware_obj.source_address:
                        mw_bytes = self.malware_analyzer.get_urlhaus_download(
                            malware_obj.source_address)
                        if not mw_bytes:
                            continue

                        mw_hash = sha256hex(mw_bytes)
                        mw_file = attack.attack_dir / \
                            f"malware/downloads/{mw_hash}"
                        ...
                        attack._malware[mw_hash] = attack._malware.pop(
                            malware_id)

                        failed_malware[mw_hash] = malware_obj
                        print(
                            f"Successfully fetched failed malware {mw_hash} for attack {attack.attack_id} from {malware_obj.source_address}")
