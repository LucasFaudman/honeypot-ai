...
web_local_path="${logdir_path}/weblogs"
...
rsync -avz -e "ssh -p $ssh_port -i $keyfile" "$user@$remote_server:$zeek_remote_logs_dir/" "$zeek_local_path"
rsync -avz -e "ssh -p $ssh_port -i $keyfile" "$user@$remote_server:$web_remote_logs_dir/" "$web_local_path"
rsync -avz -e "ssh -p $ssh_port -i $keyfile" "$user@$remote_server:$cowrie_remote_logs_dir/" "$cowrie_local_path"
rsync -avz -e "ssh -p $ssh_port -i $keyfile" "$user@$remote_server:$firewall_remote_logs_dir/dshield*" "$firewall_local_path"
rsync -avz -e "ssh -p $ssh_port -i $keyfile" "$user@$remote_server:$malware_remote_logs_dir/" "$malware_local_path"



#TIMESTAMP=$(date +%s)
ssh -i $keyfile -p $ssh_port $user@$remote_server "df -h /" \
#| tee "$logdir_path/df-$TIMESTAMP.txt"


#scp -i $keyfile  -P $ssh_port  -r "zeek/local.zeek" "$user@$remote_server:/opt/zeek/share/zeek/site/local.zeek" 
