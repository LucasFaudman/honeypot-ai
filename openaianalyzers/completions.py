...
class OpenAICompletionsAnalyzer(OpenAIAnalyzerBase):
    ...
    def get_json_result(self, messages=[], n=1, retries=0, is_retry=False, **kwargs):
        ...
        if not result:
            ...
            with db_file.open("w+") as f:
                json.dump({"messages": messages, "kwargs": kwargs,
    ...
    def explain_commands(self, commands=[], n=1, retries=0, **kwargs):
        ...
        example_commands1 = self.read_training_data(
            "explain_commands/example_commands1.sh", returnas=list)
        example_commands1 = self.index_content(example_commands1)
        example_response1 = self.read_training_data(
            "explain_commands/example_response1.json", returnas="json")

        examples = [{"input": example_commands1,
                     "response": example_response1}]
    ...
    def explain_malware(self, malware_source_code, commands=[], n=1, retries=0, **kwargs):
        ...
        example_malware1 = self.read_training_data(
            "shared/example_malware1.py")
        example_commands1 = self.read_training_data(
            "shared/example_commands1.sh", returnas=list)
        example_explanation1 = self.read_training_data(
            "shared/example_explanation.txt")

        example_input1 = {"malware_source_code": example_malware1,
                          "commands": self.index_content(example_commands1)}
        example_response1 = {
            "malware_explanation": example_explanation1, "malware_language": "python"}
    ...
    def comment_malware(self, malware_source_code, commands=[], n=1, retries=0, **kwargs):
        ...
        example_commands = self.read_training_data(
            "shared/example_commands2.sh", returnas=list)
        example_malware = self.read_training_data("shared/example_malware2.sh")
        language, commented_malware = self.read_training_data(
            "shared/commented_malware2.sh", returnas="split_firstline")
    ...
    def explain_and_comment_malware(self, malware_source_code, commands=[], n=1, retries=0, **kwargs):
        ...
        example_malware1 = self.read_training_data(
            "shared/example_malware1.py")
        example_explanation1 = self.read_training_data(
            "shared/example_explanation1.md")

        language, commented_malware1 = self.read_training_data(
            "shared/commented_malware1.py", returnas="split_firstline")
        language = language.split(":")[1].strip()

        example_input1 = {"malware_source_code": example_malware1,
                          "commands": self.index_content(example_commands1)}
        example_response1 = {"commented_code": commented_malware1,
                             "malware_explanation": example_explanation1, "malware_language": language}
    ...
    def answer_attack_questions(self, questions: list, commands=[], malware_source_code=None, n=1, retries=0, **kwargs):
        ...
        q1, a1 = self.read_training_data(
            "answer_attack_questions/example_questions1.md", returnas="split_firstline")
