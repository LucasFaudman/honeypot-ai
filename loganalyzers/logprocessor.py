...
class LogProcessor:
    ...
    def __init__(self,
                 ...
                 sort_attrs=["src_ips", "sessions",
                             "commands", "malware", "http_requests"],
                 sort_order="desc",
                 ):
    ...
    def load_attacks_from_attacks_dir(self,
            ...
            if only_attacks and attack_dir.name not in only_attacks:
                ...
                with (attack_dir / "ips.txt").open('r') as f:
                    ...
                    if num_ips > max_ips_per_attack:
                        print(
                            f"Skipped loading attack {attack_dir.name} with {num_ips} ips")
                        continue
        ...
        if max_workers < 2 or len(attack_dirs) < 2:
            ...
            print(
                f"Loading {len(attack_dirs)} attacks in parallel with {max_workers} workers.")
            print(
    ...
    def process_logs_into_source_ips(self):
        ...
        for parser_num, parser in enumerate(self.parsers):
            print(
                f"Processing Events from Parser {parser_num + 1} of {len(self.parsers)}: {parser}")
            ...
            for event in parser.logs():
                ...
                try:
                    ...
                    if event["eventid"].startswith("cowrie"):
                        ...
                        if event["eventid"].startswith("cowrie.client."):
                            ...
                            self.source_ips[event["src_ip"]].sessions[event["session"]].add_command(
                                event)
                        elif event["eventid"].startswith("cowrie.session.file_"):
                            self.source_ips[event["src_ip"]].sessions[event["session"]].add_malware(
                                event)
                        elif event["eventid"] == "cowrie.log.closed":
                            self.source_ips[event["src_ip"]].sessions[event["session"]].add_ttylog(
                                event)
                    ...
                    # Handle Zeek Events
                        ...
                        self.source_ips[event["src_ip"]].sessions[event["session"]].add_zeek_event(
                            event)
                ...
                print(
                    f"Processed {total_events} events ({cowrie_events} cowrie events, {zeek_events} zeek events). Found {source_ips_found} source ips", end='\r')
    ...
    def process_source_ips_into_attacks(self):
        ...
        self.all_attack_ids_by_type = {"malware_hash": OrderedSet(
            ()), "cmdlog_hash": OrderedSet(()), "httplog_hash": OrderedSet(())}
        for source_ip in sorted(self.source_ips.values(), key=lambda source_ip: source_ip.first_seen or datetime.now()):
            ...
            if source_ip.total_malware >= self.min_malware:
                ...
                src_ip_attack_ids_by_type['malware_hash'].update(
                    source_ip.all_malware_hashes)

            if source_ip.commands >= self.min_commands:
                self.ips_with_commands.append(ip)
                src_ip_attack_ids_by_type['cmdlog_hash'].update(
                    source_ip.all_cmdlog_hashes)
            ...
            if source_ip.http_requests >= self.min_http_requests:
                ...
                if flagged:
                    ...
                    src_ip_attack_ids_by_type['httplog_hash'].update(
                        source_ip.all_httplog_hashes)
            ...
            if not any(shared_attack_ids.values()):
                attack_id_type, attack_ids = next(
                    filter(lambda x: x[1], src_ip_attack_ids_by_type.items()))
                ...
                attack_id_type, attack_ids = next(
                    filter(lambda x: x[1], shared_attack_ids.items()))
                attack_id = attack_ids[0]
    ...
    def merge_attacks_by_shared_attrs(self):
        ...
        for (attack_id, attack), (attack_id2, attack2) in attack_combos:
            ...
            for attr in self.merge_shared_attrs:
                merge_attempts += 1
                print(
                    f"Attempting merge {merge_attempts}: {attack_id} <- {attack_id2} by {attr}", end='\r')

                shared_attr = getattr(
                    attack, "uniq_" + attr) & getattr(attack2, "uniq_" + attr)
                if shared_attr:
                    merge_successes += 1
                    ...
                    print(
                        f"Merged {attack_id} <- {attack_id2} by {attr}: {shared_attr}")
                    break
            ...
            for attr, compiled_regexes in attack_sigs.items():
                ...
                for value in getattr(attack, attr):
                    if value not in value_to_regexes:
                        value_to_regexes[value] = [
                            compiled_regex for compiled_regex in compiled_regexes if compiled_regex.search(value)]
                    ...
                    for compiled_regex in value_to_regexes[value]:
                        if not attack_sigs[attr][compiled_regex]:
                            ...
                            print(
                                f"Regex merged {attack.attack_id} into {attack_sigs[attr][compiled_regex].attack_id} on {attr}: {str(compiled_regex)}")
                            break
                    if merged:
                        break
    ...
    def print_stats(self):
        ...
        print(
            f" {len(self.ips_with_commands)} IPs with >={self.min_commands} commands")
        ...
        print(
            f" {len(self.ips_with_http_requests)} IPs with >={self.min_http_requests} http requests")
        print(
            f" {len(self.ips_with_flagged_http_requests)} IPs with flagged http requests")
        print(
            f" {len(self.benign_ips)} Benign IPs. (Generated log events but not in any attacks)")
