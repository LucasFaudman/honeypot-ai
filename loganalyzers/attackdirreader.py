...
class AttackDirReader:
    ...
    def update_attack_log_counts(self, attack, ips="all", log_filter="all"):
        if log_filter == "all":
            log_filters = ["cowrie.log", "cowrie.json",
                           "web.json", "dshield.log", "zeek.log"]
        else:
        ...
        for ip in ips:
            for log_filter in log_filters:
                ...
                for log_path in self.get_attack_log_paths(attack, ip, log_type, ext):
                    ...
                    with log_path.open("rb") as f:
                        log_counts[ip][log_filter][log_path.name] = sum(
                            1 for line in f if not line.startswith(b"#"))
            ...
            log_counts[ip]["_lines"] = sum(
                log_counts[ip][log_filter]["_lines"] for log_filter in found_log_filters)
            log_counts[ip]["_files"] = sum(
                log_counts[ip][log_filter]["_files"] for log_filter in found_log_filters)
