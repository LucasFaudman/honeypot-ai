...
read -p "Enable Json Logs? (default n) (y/n): " JSON_LOGS
if [ "$JSON_LOGS" == "y" ]; then
    echo "
    #Enable JSON Logs
...
read -p "Add Zeek Scripts? (default n) (y/n): " ADD_SCRIPTS
...
echo "Zeek service installed and started. Logs will be aggregated every $ZEEK_RUN_TIME seconds."
echo "To change the aggregation time, rerun this script or edit $ZEEK_RUN_SCRIPT_DIR/run-zeek-aggregate-logs.sh"
exit 0
