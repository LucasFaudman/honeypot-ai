...
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<USER>|$USER|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<HONEYPOT_IP>|$HONEYPOT_EXTERNAL_IP|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<SSH_PORT>|$ADMIN_PORT|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<KEYFILE>|$KEYFILE|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<FIREWALL_REMOTE_PATH>|$FIREWALL_REMOTE_PATH|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<WEBLOGS_REMOTE_PATH>|$WEBLOGS_REMOTE_PATH|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<COWRIE_REMOTE_PATH>|$COWRIE_REMOTE_PATH|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<MALWARE_REMOTE_PATH>|$MALWARE_REMOTE_PATH|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<ZEEK_REMOTE_PATH>|$ZEEK_REMOTE_PATH|" sync-logs.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<LOGS_PATH>|$LOGS_PATH|" sync-logs.sh
rm *"$SAFE_TO_DELETE_EXT"
...
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<USER>|$USER|" install-zeek-on-honeypot.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<HONEYPOT_IP>|$HONEYPOT_EXTERNAL_IP|" install-zeek-on-honeypot.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<SSH_PORT>|$ADMIN_PORT|" install-zeek-on-honeypot.sh
sed -i"$SAFE_TO_DELETE_EXT" -e "s|<KEYFILE>|$KEYFILE|" install-zeek-on-honeypot.sh
rm *"$SAFE_TO_DELETE_EXT"
...
if [ "$SELENIUM" == "y" ]; then
    python3 setup/getchromedriver.py
    unzip "$RESOURCES_PATH"/*.zip "$RESOURCES_PATH"
...
printf "./install-zeek-on-honeypot.sh to install Zeek on your Honeypot or update your Zeek settings.\n"
printf "python3 main.py to run Honeypot AI.\n"

read -p "Show help for main.py? (y/n): " SHOW_HELP
[ "$SHOW_HELP" == "y" ] && python3 main.py -h

read -p "Run sync-logs.sh now? (y/n): " RUN_SYNC_LOGS
[ "$RUN_SYNC_LOGS" == "y" ] && ./sync-logs.sh
