...
class CowrieAttackMarkdownWriter(MarkdownWriter):
    ...
    def add_time_and_date(self, md, attack: Attack):
        ...
        td_md = ""
        td_md += placeholder(f"First activity logged: {code(str(attack.start_time))}") + "\n"
        td_md += bullet(f"First session: {code(first_session.session_id)}")
        td_md += bullet(f"First IP/Port: {code(first_session.src_ip)}:{code(first_session.src_port)} -> {code(first_session.dst_ip)}:{code(first_session.dst_port)}" )
        td_md += "\n"

        td_md += placeholder(f"Last activity logged: {code(str(attack.end_time))}") + "\n"
        td_md += bullet(f"Last session: {code(last_session.session_id)}")
        td_md += bullet(f"Last IP/Port: {code(last_session.src_ip)}:{code(last_session.src_port)} -> {code(last_session.dst_ip)}:{code(last_session.dst_port)}" )
        td_md += "\n"
    ...
    def add_attack_summary(self, md, attack: Attack):
        ...
        if len(uniq_src_ips) <= 15:
            ...
            src_ip_summary = md_join(uniq_src_ips[:15], code, ", ") + f" (and {code(len(uniq_src_ips)-15)} more)" 

        if len(counts['all_src_ports']) <= 15:
            src_port_summary = md_join(counts['all_src_ports'], code, ", ")
        else:
        ...
        summary = [
            ...
            f"{code(len(counts['all_dst_ports']))} unique destination ports were targeted: {', '.join([code(port) for port in counts['all_dst_ports']])}",
            f"{code(len(counts['all_src_ports']))} unique source ports were used: {src_port_summary}",
            f"{code(len(attack.commands))} commands were input in total. {code(len(counts['all_cmdlog_ips']))} IP(s) and {code(len(counts['all_cmdlog_urls']))} URL(s) were found in the commands",
            ...
            f"This attacks was recorded in {code(len(log_types))} log types: " + md_join(log_types, code, ', '),
            f"A total of {code(log_counts['all']['lines'])} log events were logged in {code(len(log_names))} log files: " + md_join(log_names, code, ', '),
        ]
        
        summary_long = [
            f"This attack was carried out by a {code(attack.num_source_ips)} unique {bold('source IP')} address(es):",
                [code(src_ip) for src_ip in attack.source_ips],

            f"{code(attack.num_uniq_src_ports)} unique {bold('source ports')} were used:",
                [*[f'Src Port: {code(port)} Used {code(count)} times' 
                 for port, count in attack.counts["src_ports"].items()
                ], 
                f"(and {code(len(uniq_src_ips)-15)} more)" if len(uniq_src_ips) > 15 else None             
                ],

            f"{code(attack.num_uniq_dst_ports)} unique {bold('destination ports')} were targeted:",
                [f'Dst Port: {code(port)} Used {code(count)} times' 
                 for port, count in attack.counts["dst_port"].items()
                ],

            f"A total of {code(attack.num_sessions)} sessions were logged:", 
                [f"{code(attack.num_login_sessions)} sessions were {bold('successful logins')}.",
                 f"{code(attack.num_sessions - attack.num_login_sessions)} sessions were {bold('failed logins')}.",
                 f"{code(attack.num_command_sessions)} sessions had commands.",
                 f"{code(attack.num_malware_sessions)} sessions had malware.",
                ],
        ...
        md += h2("Summary") 
        md += nested_list(summary)
        md += h2("Summary Long") 
        md += nested_list(summary_long, style_dict={1:code, })
        return md
    ...
    def add_relevant_logs(self, md, attack: Attack):
        ...
        #logs_md += bullet(f"This attack was generated a total of {code(log_counts['all']['lines'])} log lines in {code(log_counts['all']['files'])} log files: {code(', '.join(log_types))}\n")
        ...
        if "dshield.log" in log_types:
            logs_md = self.add_dshield_logs(logs_md, attack, ip="all", n_lines=None)

        if "cowrie.log" in log_types:
            logs_md = self.add_cowrie_logs(logs_md, attack, ip="all", n_lines=None)

        if "web.json" in log_types:
            logs_md = self.add_web_logs(logs_md, attack, ip="all", n_lines=None)
    ...
    def add_cowrie_logs(self, md, attack: Attack, ip="all", n_lines=None):
        ...
        for ext in ('.log', '.json'):
            ...
            log_lines = attack.get_log_lines(first_command_session.src_ip, f"cowrie{ext}", session_filter,  n_lines)
            codeblock_md = codeblock(log_lines, codeblock_lang)


            md += collapseable_section(codeblock_md, f"Cowrie {ext} Logs for {first_command_session.session_id}", 3)
            md += f"{placeholder('COMMENTARY ON LOGS')}\n"
        return md
    ...
    def add_ip_and_port_tables(self, md, attack: Attack):
        ip_md = ""#h2("IP and Ports")
        n = 10

        ip_md += most_common_table("Source IP", attack.counts["src_ips"], n)
        ip_md += most_common_table("Destination IP", attack.counts["dst_ips"], n)
        ip_md += most_common_table("Source Port", attack.counts["src_ports"], n)
        ip_md += most_common_table("Destination Port", attack.counts["dst_ports"], n)
    ...
    def add_custom_scripts(self, md, attack: Attack):
        ...
        script_md = table(["Script", "Description"], [[self.script_link(script), description] for script, description in scripts.items()])
        md += collapseable_section(script_md, "Custom Scripts Used To Generate This Report", 1)
    ...
    def command_analysis(self, attack: Attack):
        ...
        md = h1("Commands Used")
        md += placeholder(f"This attack used a total of {code(len(commands))} inputs to execute the following {code(len(split_commands))} commands:\n")
        
        #md += h3("Raw Command Inputs")
        ...
        md += collapseable_section(commands_explained_md, "Commands Explained", 2) + "\n"
    ...
    def malware_analysis(self, attack: Attack):
        ...
        md += placeholder(f"This attack downloaded {code(len(malware))} raw malware samples which can be standardized into {code(len(standardized_malware))} samples:\n")
        ...
        for n, sample in enumerate(standardized_malware.items()):
            ...
            #mw_md = h4(f"Malware Sample {bold(n)}/{len(malware)} Sha256 HASH: {code(mwobj0.shasum)}")
            ...
            if len(mwobj_list) > 1:
                ...
                mw_md += unordered_list([mwobj.shasum for mwobj in mwobj_list[1:]], style_fn=code)

            label = f"Raw Malware Sample {n}/{len(standardized_malware)} Sha256 HASH: {code(mwobj0.shasum)}"
